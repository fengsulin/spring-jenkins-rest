...
import com.example.ci.annotion.PathParser;
...
public interface JobsApi extends BaseApi{
    ...
    /**
     * @description: 用户程序内部执行调用，optionalFolderPath会被自定义解析成完整的job路径
     * @author: FSL
     * @date: 2023/3/24
     ...
     */
    @PostExchange(value = "{optionalFolderPath}/buildWithParameters",contentType = MediaType.APPLICATION_FORM_URLENCODED_VALUE)
    ResponseEntity<String> externalBuildWithParameters(@PathVariable @PathParser String optionalFolderPath,
                                               @PathVariable("name") String jobName,
                                               @RequestParam("json") String json);
